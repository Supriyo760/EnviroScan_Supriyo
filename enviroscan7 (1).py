...
# --- Helper Functions ---
# (The helper functions remain unchanged from the previous version)
...
with st.sidebar:
    ...
    with st.expander("Time Range", expanded=True):
        ...
        with col4:
            start_date = st.date_input("Start Date", value=datetime(2025, 9, 1), key="start_date", help="Select start date")
        with col5:
            end_date = st.date_input("End Date", value=datetime(2025, 9, 15), key="end_date", help="Select end date")
        time_range = f"{start_date} to {end_date}"

    uploaded_file = st.file_uploader("Upload CSV File", type=["csv"], key="file_uploader", help="Upload your environmental data CSV")
...
# --- Right-Side Pollution Source Info Panel ---
with right_col:
    st.markdown('<div class="card">', unsafe_allow_html=True)
