...
def build_dataset(city, lat, lon, aq_csv_path, openweather_key):
    ...
    try:
        ...
        print("\n📊 AQ Dataset Preview (horizontal format):")
        print(df_horizontal.head(10))
    except Exception as e:
        print(f"⚠️ Could not pivot for horizontal view: {e}")
        print(df_aq.head())
    ...
    try:
        ...
        print(f"⚠️ OSM data fetch failed: {e}")
    ...
    print("\n📊 Metadata + Weather + OSM:\n", meta)

    return df_aq, meta

def save_datasets(df, filename):
    if df is None or (isinstance(df, pd.DataFrame) and df.empty):
        print(f"⚠️ {filename} is empty. Skipping save.")
        return
    if isinstance(df, dict):
        df = pd.DataFrame([df])
    df.to_csv(f"{filename}.csv", index=False)
    df.to_json(f"{filename}.json", orient="records", indent=2)
    print(f"💾 Saved {filename}.csv and {filename}.json")

def consolidate_dataset(df_aq, df_meta, filename):
    if df_aq is None or df_aq.empty:
        print("⚠️ AQ dataset empty, skipping consolidation.")
        return
    for k, v in df_meta.items():
        df_aq[k] = v
    df_aq.to_csv(f"{filename}.csv", index=False)
    print(f"💾 Consolidated dataset saved as {filename}.csv")
...
print("💾 Cleaned dataset saved as cleaned_environmental_data.csv")
...
print("✅ Source labels assigned using rule-based simulation")
print(df["pollution_source"].value_counts())

# Save labeled dataset
df.to_csv("source_labeled_data.csv", index=False)
print("💾 Source-labeled dataset saved as source_labeled_data.csv")
...
print("💾 Best model saved as pollution_source_model.pkl")
