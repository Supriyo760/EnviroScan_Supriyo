...
def label_source(row):
    ...
    # Adjusted thresholds for scaled data
    if pd.notna(pm25) and pm25 > 0 and industries > 0:
        return "Industrial"
    elif pd.notna(pm25) and pm25 > 0 and roads > 0:
        return "Traffic"
    elif farms > 0:
        return "Agricultural"
    else:
...
if uploaded_file:
    ...
    if not df_aq.empty:
        ...
        # Median Values per Station Preview (BEFORE scaling)
