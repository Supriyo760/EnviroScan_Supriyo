...
def build_dataset(city, lat, lon, aq_csv_file, openweather_key):
    try:
        ...
        # Try different skiprows values to skip metadata
        ...
        for skiprows in skiprows_values:
            try:
                ...
                st.write(f"Successfully loaded CSV with skiprows={skiprows}")
                break
            except Exception as e:
                st.write(f"Failed with skiprows={skiprows}: {e}")
        if df_aq is None:
            st.error("⚠️ Failed to load CSV with any skiprows value (0-5)")
            return pd.DataFrame(), {}
